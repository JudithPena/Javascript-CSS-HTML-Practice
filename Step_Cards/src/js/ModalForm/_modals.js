...
class Modals {
    ...
    createFirstModal() {
        ...
        this.filterForm.after(this.wrapperModal);
    ...
    selectDoctors() {
        ...
        selectDoctors.addEventListener("change", (e) => {
            this.doctorListener(e.target.options.selectedIndex);
        });
    }

    getIndexDoctors(doctor) {
        console.log(doctor);
        if (doctor === "Кардиолог") {
            this.doctorListener(1);
        }
        if (doctor === "Стоматолог") {
            console.log("doctogjhgkhjr");
            this.doctorListener(2);
        }
        if (doctor === "Терапевт") {
            this.doctorListener(3);
        }

    }

    doctorListener(selectedIndex) {
        if (this.response) {
            this.container = document.querySelector(`#card_${this.response.id}`);
        } else {
            this.container = document;
        }

        const inputsAll = this.container.querySelector('.inputs-all');
        const inputsTherapist = this.container.querySelector('.inputs-therapist');
        const inputsDentist = this.container.querySelector('.inputs-dentist');
        const inputsCardiologist = this.container.querySelector('.inputs-cardiologist');
        const submitWrap = this.container.querySelector('.submit-wrap');
    ...
    createVisit() {
        ...
        formVisit.addEventListener('submit', (e) => {
            e.preventDefault();
            const card = this.fillForm(formVisit);
            this.api.addCard(card, this.wrapperModal);
        });
    }

    updateVisit(id, modal, cardDiv) {
        const formVisit = document.getElementById("appointment-fixed-block");
        formVisit.addEventListener('submit', (e) => {
            e.preventDefault();
            const card = this.fillForm(formVisit);
            this.api.putCard(id, card, this.wrapperModal, cardDiv);
        });
    }
    ...
    fillForm(formVisit) {
        return {
            ...
            selectStatusVisit: formVisit.elements["select-status-visit"].options[formVisit.elements["select-status-visit"].selectedIndex].text,
            ...
            additionalСomments: formVisit.elements["additional-comments"].value,
        };
    }
